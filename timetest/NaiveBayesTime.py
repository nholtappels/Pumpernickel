...
def loadIrisData(filename):
	dataset = np.asarray(np.loadtxt(filename, delimiter=',', skiprows=1))
	np.random.shuffle(dataset)

	#extract target:
	targets = dataset[:,4]
	targets = targets.reshape((dataset.shape[0],1))

	# extract data:
	data = np.array(dataset[:,:4])
